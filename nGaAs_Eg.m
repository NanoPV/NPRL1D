function [Eg_GaAs, ni_GaAs, mu, erel]=nGaAs_Eg(Nd,Na, T)
...
dEg_GaAs=2e-11*Na.^(.5); %in eV, p-type gap narrowing from Marti
dEgn_GaAs=.125/erel*abs(1-Nd/Ncr).^(1/3); %eV (intrinsic (n-type?) gap narrowing from Marti)
...
mu=mumaxp*(Tn.^(-2.3))./(1+Nd./(Nrefp*Tn^3.8)) %nGaAs base (minority holes in n type)
