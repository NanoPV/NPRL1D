...
%WavelengthEQEIQERSRdata = xlsread('C:\Users\Zac Bittner\Documents\VM 2013-4-10\UCLAQE\UCLA2013-2-21.xls'); %file location of spectral response/EQE data
...
c=1/3;
mumaxn=22600*0.52 + 2500*(1-0.52)%-6933*c^3+3581*c^2+76*c+9157;
...
%Holes........
mumaxp=250*0.52 + 150*(1-0.52)%400; %[cm2/Vs]
Nrefp=1e17; %[cm-3]
%  %Hole aux equation
...
mub=606;%muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Ma./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))) %pGaAs emitter (minority e- in p type)
mui=muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Mi./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))) %pGaAs i-region
mue=25;%mumaxp*(Tn.^(-2.3))./(1+Md./(Nrefp*Tn^3.8)) %nGaAs base (minority holes in n type)
...
Sw=1E12; %1e8; %front surface InGaP/air interface  %%%% VARIABLE!!!!!!!!!!!!!!!
Se=1000; %emitter/window interface recombination  %%%% VARIABLE!!!!!!!!!!!!!!!
Sb=1000; %base back surface recombination  %%%% VARIABLE!!!!!!!!!!!!!!!
dataS=[Sw,Se,Sb];

%minority carrier lifetimes
tauw = 8e-13;%s%2E-13 (2012-3-6 .01E-11)
taue = 2E-11%1E-11;%1.4E-11% (2012-3-6 1E-10)
taub = 806E-12%15e-8; %s3.5E-11% (2012-3-6 12E-10)
%tauip = 1.1E-5%1.1E-15 %1.3E-15%1.2E-15
%tauie = 2.6E-5 %2.0E-11%2.1E-11
%taui=tauip;
% tausub1
...
ni_w = 1500;
ni_e = 1.23E6;%ni_GaAs; %Ga(x)In(1-x)P, x=51.5% Eg=1.85
ni_b = 1.23E6;%ni_GaAs;
...
a_c=a_InGaAscap;
a_w=a_In35AlAs;%a_iAlGaAsUCLA; %absorption in windowchanged Aw to a_w
...
QEe=(1-ref).*exp(a_c*1.5E-6).*exp(-a_w.*t_w).*a_e.*Le./(a_e.^2.*Le.^2-1).*((a_e.*Le+Se.*taue./Le.*(1-exp(-a_e.*t_e).*cosh(t_e./Le))-exp(-a_e.*t_e).*sinh(t_e./Le))./(Se.*taue./Le.*sinh(t_e./Le)+cosh(t_e./Le))-a_e.*Le.*exp(-a_e.*t_e))+QEw./(Se.*taue./Le.*sinh(t_e./Le)+cosh(t_e./Le));
...
%from Nelson:
QE_w=(1-ref).*exp(-a_c*1.5E-6).*a_w.*Lw./(a_w.^2.*Lw.^2-1).*( (a_w.*Lw+Sw.*tauw./Lw.*(1-exp(-a_w.*t_w).*cosh(t_w./Lw))-exp(-a_w.*t_w).*sinh(t_w./Lw))./(Sw.*tauw./Lw.*sinh(t_w./Lw)+cosh(t_w./Lw))-a_w.*Lw.*exp(-a_w.*t_w) );
QE_e = (1-ref).*exp(-a_c*1.5E-6) .* exp(-a_w.*t_w) .* ( S./(S.^2 - 1)) .*  (  ((V+a_e.*Le) - exp(-a_e.*(t_e-wi_e)).*(V.*cosh((t_e-wi_e)./Le)+sinh((t_e-wi_e)./Le) ) ) / ( V.*sinh((t_e-wi_e)./Le)+cosh((t_e-wi_e)./Le) ) - (a_e.*Le.*exp(-a_e.*(t_e-wi_e))) );
QE_scr = (1-ref).*exp(-a_c*1.5E-6) .* exp(-a_w.*t_w) .* ( 1-exp(-a_i.*(t_i+Wd)) ) .* exp( -a_e .*(t_e-wi_e) );
...
NQEb=(1-ref).*exp(-a_c*1.5E-6).*exp(-a_w.*t_w).*exp(-a_i.*t_i).*a_b*Lb./(a_b.^2*Lb^2-1).*exp(-a_e*t_e-a_b*wi_b).*(a_b*Lb-(Sb*Lb/Db*(cosh((t_b-wi_b)/Lb)-exp(-a_b*(t_b-wi_b)))+sinh((t_b-wi_b)/Lb)+a_b.*Lb.*exp(-a_b*(t_b-wi_b)))./(Sb*Lb/Db*sinh((t_b-wi_b)/Lb)+cosh((t_b-wi_b)/Lb)));
...
Jsc=sum(F.*dlambda.*QE_t.*Lambda./1240)*1000
...
%  output1=[Lambda QEw QEe QEscr QEb SRw SRe SRb SRscr SR]; %Hovel SR outputs
    ...
    figure(1)
    plot(Lambda, [QE_w QE_e QE_scr NQEb QE_t ref], Wave,EQE,'*k')
    
    title('\fontsize{18}QE Fit')
    xlabel('\fontsize{18}Wavelength (nm)');
    set(gca,'XLim',[400 1000],'Layer','top')
    ylabel('\fontsize{18}QE');
    YLim([0 1])
    legend('\fontsize{12}Window','\fontsize{12}Emitter','\fontsize{12}SCR','\fontsize{12}Base', '\fontsize{12}TOTAL', '\fontsize{12}Ref', '\fontsize{12}Experimental pin control' ) %
