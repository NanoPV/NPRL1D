...
load('~/Google Drive/NPRL1D/QEworkspaceInGaAs.mat'); %absorption, spectra, and reflectance data
...
mue=200;%ioffe hole in InGaAs %muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Ma./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))) %pGaAs emitter (minority e- in p type)
mui=1.5E4;%muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Mi./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))); %pGaAs i-region
mub=3E3;%mumaxp*(Tn.^(-2.3))./(1+Md./(Nrefp*Tn^3.8)) %nGaAs base (minority holes in n type)
...
ni_w = 1500;
ni_e = ni_InGaAs; %Ga(x)In(1-x)P, x=51.5% Eg=1.85
ni_b = ni_InGaAs;
...
a_w=a_InPBYU; %absorption in windowchanged Aw to a_w
a_e=a_InGaAsMod; %absorption in emitterchanged An to a_e
a_i=a_InGaAsMod; %absorption in i-regionGaAs changed Ai to a_i
a_b=a_InGaAsMod; %absorption in baseGaAs changed Ap to a_b
...
dlambda=2;
Jsc=42.46%sum(F.*dlambda.*QE_t.*Lambda./1240)*1000
...
J=(Jp+Jn)*(exp(V./.026)-1);
Jlight=Jsc-J.*1000;
Voc=.026*log(Jsc/((Jp+Jn)*1000))
pmax=max(Jlight.*V);
fillfactor=pmax/(Jsc*Voc)
Efficiency=pmax/136.6
...
%  output1=[Lambda QEw QEe QEscr QEb SRw SRe SRb SRscr SR]; %Hovel SR outputs
    ...
    figure(1)
    plot(Lambda, [QE_w QE_e QE_scr QE_b QE_t ref], Wave, EQE, '*k')
    
    title('\fontsize{18}QE Fit')
     xlabel('\fontsize{18}Wavelength (nm)');
    set(gca,'XLim',[300 2000],'Layer','top')
    ylabel('\fontsize{18}QE');
    ylim([0 1])
    legend('\fontsize{18}Window','\fontsize{12}Emitter','\fontsize{12}SCR','\fontsize{12}Base', '\fontsize{12}TOTAL', '\fontsize{12}Reflectance', '\fontsize{12}EQE' ) %
    legend('Location',['NorthWest']) %best
   
     figure(2)
    plot(V, [Jlight], expV, expJ)
    
    title('\fontsize{18}AM0 JV')
    xlabel('\fontsize{18}Voltage (V)');
    set(gca,'XLim',[0 0.5],'Layer','top')
