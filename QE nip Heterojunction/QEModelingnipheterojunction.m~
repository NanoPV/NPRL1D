% function [ output ] = QEpol(tsrhe, tsrhb,SR09A161, SR10A018C3,Lambda,Fsun, Ref,Aryanadj2,Aw,Ne,Naw,Nb,wi_b,wi_e,Wd,taue,taub,tauw,mue,mub,muw,Le,Lb,Lw, De,Db,Dw,Se,Sb,Sw,Vt,q,T,C,t_e,tauw,t_b,ni,nib,Vbi,t_i,area,pol)
...
load( '/Users/zac/Dropbox/VM Backups/VM 2013-4-10/Kerestes QE/QE nip Heterojunction/QEworkspace4.mat'); %absorption, spectra, and reflectance data
%WavelengthEQEIQERSRdata = xlsread('C:\Users\Zac Bittner\Documents\MATLAB\Kerestes QE\GaAs60XR1.xls'); %file location of spectral response/EQE data
...
muw=muminn*Tn^(-.57)+((mumaxn-muminn)*Tn^(-2.3))/(1+(Maw/(Nrefn*Tn^(-2.4)))^(chi*Tn^(-.146))); %InGaP window  (minority holes in ntype)
mub=muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Ma./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))) %pGaAs base (minority e- in p type)
mui=muminn*Tn.^(-.57)+((mumaxn-muminn)*Tn^(-2.3))./(1+(Mi./(Nrefn*Tn^(-2.4))).^(chi*Tn^(-.146))); %pGaAs i-region
mue=mumaxp*(Tn.^(-2.3))./(1+Md./(Nrefp*Tn^3.8)) %nGaAs base (minority holes in n type)
musub=mumaxp*(Tn^(-2.3))/(1+Msub/(Nrefp*Tn^3.8));
muhet=(491.5-20)/(1+(Ne/1.48E17)^.38);
%Diffusivity
Dw=Vt*muw;
De=Vt*muhet
Di=Vt.*mui;
...
Sw=1E18; %1e8; %front surface InGaP/air interface  %%%% VARIABLE!!!!!!!!!!!!!!!
...
tauw = 2e-13;%s%2E-13 2E-15
taue = 8E-14%3.2E-14;%1.4E-11%3.2E-11%8E-16
taub = 5E-10%4e-9; %s3.5E-11%3.5E-11
...
dEg_GaAs=2e-11*Ne.^(.5); %in eV, p-type gap narrowing from Marti
dEgn_GaAs=.125/erel_e*abs(1-Nb./Ncr).^(1/3); %eV (intrinsic (n-type?) gap narrowing from Marti)
ni_GaAs=((Nc_GaAs*Nv_GaAs)^.5)*exp((-Eg_GaAs-dEgn_GaAs)./(2*Vt)); %ni in base

ni_w = 1500;
ni_e = 1500; %Ga(x)In(1-x)P, x=51.5% Eg=1.85
ni_b = ni_GaAs;

%built in junction voltage
Ef_b=4.07+Eg_GaAs/2+Vt*log(Nb/ni_GaAs)
Ep_b=Eg_GaAs/2-Vt*log(Nb/ni_GaAs)
Ef_e=4.1+1.9+Vt*log(Ne/282)
En_e=1.9/2-Vt*log(Ne/282)
Vbi=Eg_GaAs-Ep_b-En_e
p_a_if=Nv_GaAs*exp(-(Ep_b+Vbi)/Vt)
%Vbi=Vt*log(Nb.*Ne./ni_e.^2);
%Vbin=Vt*log(Nb/ni_b);
%Vbip=Vt*log(Ne/ni_e);
%Vbisep=Vt*( log(Nb./ni_b)+log(Ne./ni_e) );
%Vbi=Vbisep

% Wdi=(2*erel_i*e0/q*(Ne+Nb)./(Ne.*Nb).*(Vbi-.95)).^.5;
% WdiBart=(t_i.^2 + (2.*erel_i.*e0./q) .* (Ne+Nb)./(Ne.*Nb) .* (Vbi-0) ).^.5 - t_i; %from Bart
% % Wdi=WdiBart;
% Vpt = t_i.^2 * 1e15* q ./ (2*erel_i*e0);

wi_e=1./Ne.*sqrt(2*erel_e*e0*Vbi./(q*(1./Ne+1./Nb)));%changed wp to wi_e
wi_b=1./Nb.*sqrt(2*erel_b*e0*Vbi./(q*(1./Ne+1./Nb)));%changed wn to wi_b
Wd=wi_e+wi_b;


%Absorption Coefficients
a_w=a_AlInPEPI; %absorption in windowchanged Aw to a_w
a_e=a_InGaPEPI; %absorption in emitterchanged An to a_e
...
QE_e = (1-ref) .* exp(-a_w.*t_w) .* ( S./(S.^2 - 1)) .*  (  ((V+Vhet+a_e.*Le) - exp(-a_e.*(t_e-wi_e)).*((V+Vhet).*cosh((t_e-wi_e)./Le)+sinh((t_e-wi_e)./Le) ) ) / ( (V+Vhet).*sinh((t_e-wi_e)./Le)+cosh((t_e-wi_e)./Le) ) - (a_e.*Le.*exp(-a_e.*(t_e-wi_e))) );
QE_scr = (1-ref) .* exp(-a_w.*t_w) .* ( 1-exp(-a_i.*(t_i)-a_b.*wi_b-a_w.*wi_e )) .* exp( -a_e .*(t_e-wi_e) );%*(1-1.6E-19*Si/(t_e+Wd))
...
QE_surface=zeros(941,1);
QEt=QE_e+QEw+QE_scr+NQEb;
jphscr=sum(F.*dlambda.*QEt.*Lambda./1240)
QE_surface = 1.6E-19*p_a_if*Si/jphscr;
QEscr=QE_scr*(1-QE_surface);
...
%  output1=[Lambda QEw QEe QEscr QEb SRw SRe SRb SRscr SR]; %Hovel SR outputs
    ...
    figure(1)
    plot(Lambda, [QE_w QE_e QE_scr NQEb QE_t], Wave,EQE,'*k')
    
    title('\fontsize{18}QE Fit')
    xlabel('\fontsize{18}Wavelength (nm)');
    set(gca,'XLim',[300 1100],'Layer','top')
    ylabel('\fontsize{18}QE');
    YLim([0 1])
    legend('\fontsize{12}Window','\fontsize{12}Emitter','\fontsize{12}SCR','\fontsize{12}Base', '\fontsize{12}TOTAL','\fontsize{12}Interface ', '\fontsize{12}Experimental Data' ) %
