...
load('~/Dropbox/VM Backups/VM 2013-4-10/Kerestes QE/QEworkspace3.mat'); %absorption, spectra, and reflectance data
...
tauw = 5e-14;%s%2E-13
taue = 1.3E-9;%1.4E-11%3.2E-11
taub = 3E-8%10E-11%10e-9; %s3.5E-11%3.5E-11
...
QE_scr = scr_m*(1-ref) .* exp(-a_w.*t_w) .* ( 1-exp(-a_i.*(t_i+Wd)) ) .* exp( -a_e .*(t_e-wi_e) );
...
%Lambda2(1)=[];
dLambda=Lambda(1:end)-Lambda2;
dLambda(1)=0.5;
Jsc=sum(F.*dLambda.*QE_t.*Lambda./1240)
Je=q*De*ni_e^2/(Ne_a*Le)*((sinh((t_e-wi_e)/Le)+Se*Le/De*cosh((t_e-wi_e)/Le))/(cosh((t_e-wi_e)/Le)+Se*Le/De*sinh((t_e-wi_e)/Le)))%*(exp(V./.026)-1)
Jb=q*Db*ni_b^2/(Nb_d*Lb)*((sinh((t_b-wi_b)/Lb)+Sb*Lb/Db*cosh((t_b-wi_b)/Lb))/(cosh((t_b-wi_b)/Lb)+Sb*Lb/Db*sinh((t_b-wi_b)/Lb)))%*(exp(V./.026)-1)
V=linspace(0,1.2,121);
J=(Je+Jb)*(exp(V./.026)-1);
Jlight=(Jsc-J).*1000;
Voc=.026*log(Jsc/(Je+Jb))
...
%  output1=[Lambda QEw QEe QEscr QEb SRw SRe SRb SRscr SR]; %Hovel SR outputs
    ...
    ylim([0 .8])
    legend('\fontsize{12}Window','\fontsize{12}Emitter','\fontsize{12}SCR','\fontsize{12}Base', '\fontsize{12}TOTAL', '\fontsize{12}Experimental' ) %
    ...
    set(gca,'XLim',[0 1.2],'Layer','top')
    ylabel('\fontsize{18}Current Density (mA/cm^2)');
    ylim([0 60])
    legend('\fontsize{12}Model')
    legend('Location',['NorthWest']) %best
